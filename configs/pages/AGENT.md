...
- `pages.steps.yml` fragment: builds UnoCSS if enabled, `bun run build`, ensures `apps/example/public` exists, configures base path for `username.github.io/repo-name`
- Official actions: `configure-pages@v5`, `upload-pages-artifact@v3`, `deploy-pages@v4` — deploy targets `github-pages` environment
- Other configs can contribute `pages.steps.yml` fragments (e.g. unocss builds CSS for Pages)
