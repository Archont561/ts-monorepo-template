...
- Baseline: **0 errors, 19 warnings, 14 infos** across 138 checked files. Warnings are expected — do not "fix" them into errors, and do not let the error count rise above zero.
...
- The 19 warnings are concentrated in `apps/example/tests/integration.test.ts` (non-null assertions), `configs/unocss/src/setup.ts` and `configs/badges/src/cli.ts` (string concatenation, literal keys). They are pre-existing and not gated.
