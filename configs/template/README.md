...
Configs with `"default": "always"` are kept — except `selfDestruct` ones, which are always removed. The template config's `always` removals also prune the template's own development state: the docs app (`apps/template-docs`), its workflow, the `docs:*` root scripts, and pending `.changeset/*.md` — a generated project starts with a clean changeset slate, keeping only the scope-replaced `config.json`.
