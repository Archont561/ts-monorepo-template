...
- `docs:sync` currently regenerates `ci.yml`, `release.yml`, `native.yml`, `stale.yml`, `dependabot-auto-merge.yml` and `.github/dependabot.yml`, and skips `pages.yml`/`coverage.yml` while the docs app exists.
...
| this change | docs site moved from root `docs/` to `apps/template-docs` (a real workspace app); removal is fully data-driven (`extraRemovals` + `scriptsToRemove`), `aggregate.ts` detects the app at its new path, and `mdocs site` builds it through Turbo with coverage/demo folded into `dist/` post-build |
