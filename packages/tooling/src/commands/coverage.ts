...
export function coveragePackages(root = "."): CoveragePackage[] {
  ...
  for (const base of [...REPORT_DIRS, "configs"]) {
...
export function renderCodecovYaml(packages: CoveragePackage[], threshold: number): string {
  const lines = [
    "# Generated by `mcoverage sync` (configs/coverage) — do not edit.",
 ...
 * merged total. Rendered by `mcoverage summary --markdown`.
 */
...
const summaryCommand = defineCommand({
  ...
  run({ args }) {
    ...
    if (args.markdown) {
      ...
      const rows = coverageReports(".", true).map((report) => ({
