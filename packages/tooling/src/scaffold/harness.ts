...
export class TemplateHarness {
  ...
  constructor(options: HarnessOptions = {}) {
    // Resolve the monorepo root by walking up to the workspace manifest rather
    // than counting `..` segments: this file moved from configs/template/src/
    // to packages/tooling/src/scaffold/, so a hardcoded depth would be wrong.
    this.templateRoot = options.templateRoot ?? repoRoot();
    this.excludes = [
      ".git",
      "node_modules",
      ".turbo",
