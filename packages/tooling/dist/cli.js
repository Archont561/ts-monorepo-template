...
\u2705 Package health OK (${e.length} package(s))`),process.exit(0)}}),Vs=Bun.fileURLToPath(import.meta.resolve("bunup/package.json").replace("package.json","dist/cli/index.js")),Us=N({name:"build",version:"1.0.0",description:"Bunup wrapper \u2014 bundler owned by @myorg/tooling, use m build not bunup",binPath:"bun",configArgs:[Vs],argsName:"entry",argsDescription:"Entry files or bunup args",subCommands:{health:mt}}),Hs=Us});var qs;var hn=f(()=>{dt();qs=mt});import{existsSync as bn,readdirSync as Ws,rmSync as Js}from"fs";var{which:Ks}=globalThis.Bun;async function vn(){if(!Ks("bun"))console.error("m bun coverage needs `bun` on PATH."),process.exit(1);console.log(`Running per-package coverage via turbo...
`),process.exit(g([...Ys,"coverage"]))}function yn(){let e=["apps","packages","configs"],t=0;for(let n of e){if(!bn(n))continue;for(let o of Ws(n,{withFileTypes:!0})){if(!o.isDirectory())continue;let s=`${n}/${o.name}/node_modules`;if(!bn(s))continue;Js(s,{recursive:!0,force:!0}),t++}}console.log(`\uD83E\uDDF9 Removed ${t} workspace node_modules dir(s) (root node_modules kept)`)}var gt,Ys,ne,zs,Qs,Zs,ke,Xs,ei,ti,ni,oi,si;var wn=f(()=>{I();k();gt=P("bunfig.toml"),Ys=["bun",`${U()}/src/cli.ts`,"turbo"];ne=v("bun"),zs=["coverage","test","clean:modules"],Qs=ne.includes("--help")||ne.includes("-h"),Zs=ne.includes("--version")||ne.includes("-v"),ke=ne[0],Xs=process.argv.slice(2).includes("bun");if(Xs&&ke&&!zs.includes(ke)&&!ke.startsWith("-")&&!Qs&&!Zs){let e=ke==="test"?["bun",ke,`--config=${gt}`,...ne.slice(1)]:["bun",...ne];process.exit(g(e))}ei=l({meta:{name:"coverage",description:"Run per-package coverage via turbo then merge LCOV"},run:async()=>{await vn()}}),ti=l({meta:{name:"clean:modules",description:"Remove workspace node_modules dirs (keeps the root one)"},run(){yn(),process.exit(0)}}),ni=l({meta:{name:"test",description:"Run bun test with shared bunfig.toml config"},run(){let e=v("test");process.exit(g(["bun","test",`--config=${gt}`,...e]))}}),oi=l({meta:{name:"bun",version:"1.0.0",description:"Bun wrapper \u2014 injects shared bunfig.toml for test, provides coverage merging"},subCommands:{coverage:ei,test:ni,"clean:modules":ti},async run(){let e=v("bun"),t=e[0];if(t==="coverage"){await vn();return}if(t==="clean:modules")yn(),process.exit(0);let n=t==="test"?["bun",t,`--config=${gt}`,...e.slice(1)]:["bun",...e];process.exit(g(n))}}),si=oi});var ii;var kn=f(()=>{I();k();ii=l({meta:{name:"test",description:"Run bun test with the shared bunfig.toml config"},args:{args:{type:"positional",description:"Extra args for bun test",required:!1}},run(){let e=process.argv.slice(2),t=e.lastIndexOf("test"),n=t===-1?[]:e.slice(t+1);process.exit(g(["bun","test",`--config=${P("bunfig.toml")}`,...n]))}})});var S="coverage/lcov.info",ue="coverage/rust-lcov.info",W="coverage/html",xe=80;var xn=()=>{};import{existsSync as j,mkdirSync as Sn,readdirSync as ri,readFileSync as $n,renameSync as ft,writeFileSync as ai}from"fs";import{dirname as ci,join as Cn}from"path";var{which:li}=globalThis.Bun;function J(e){return Boolean(li(e))}function Fe(e=S){if(!j(e))return null;let t=0,n=0;for(let o of $n(e,"utf8").split(`
...
Then run: m ci:lint
`,er,tr,nr,or;var He=f(()=>{I();k();de();zi=["-P","ubuntu-latest=catthehacker/ubuntu:act-latest","--container-architecture","linux/amd64"];er=V({name:"lint",description:"Validate workflows via actionlint with shared config",argsDescription:"Extra args for actionlint",spawn:kt}),tr=V({name:"act",description:"Run GitHub Actions locally via act with baked-in flags",argsDescription:"Extra args for act",spawn:Ue}),nr=l({meta:{name:"ci",version:"1.0.0",description:"CI tooling for GitHub Actions \u2014 lint workflows and run locally with act"},subCommands:{lint:er,act:tr},run(){let e=v("ci");if(e.length>0&&e[0]?.startsWith("-"))Ue(e);else console.log(`
...
Examples:
  ...
  m ci act push -n
  m ci act push
`)}}),or=nr});var sr;var Fn=f(()=>{k();He();sr=l({meta:{name:"ci:lint",description:"Validate workflows via actionlint with shared config"},args:{args:{type:"positional",description:"Extra args for actionlint",required:!1}},run(){process.exit(kt(v("ci:lint")))}})});var ir;var Vn=f(()=>{k();He();ir=l({meta:{name:"ci:local",description:"Run the push workflow locally via act"},args:{args:{type:"positional",description:"Extra args for act",required:!1}},run(){process.exit(Ue(["push",...v("ci:local")]))}})});function xt(e){return Y("gitleaks",(t)=>g([t,...e]))}var rr,ar,cr,lr;var Un=f(()=>{k();de();rr=V({name:"detect",description:"gitleaks detect --source . --no-git (scan repo)",prefixArgs:["detect"],defaultArgs:["--source",".","--no-git","--verbose"],spawn:xt}),ar=V({name:"protect",description:"gitleaks protect --staged (scan staged changes, pre-commit)",prefixArgs:["protect"],defaultArgs:["--staged","--verbose"],spawn:xt}),cr=l({meta:{name:"gitleaks",version:"1.0.0",description:"Gitleaks wrapper \u2014 secret scanning, defensive (skips if binary missing)"},subCommands:{detect:rr,protect:ar},run(){let e=v("gitleaks");if(e.length===0)console.log(`
...
`),process.exit(0);process.exit(xt(e))}}),lr=cr});import{existsSync as pr}from"fs";var{which:ur}=globalThis.Bun;function St(e){return Y("trivy",(t)=>g([t,...e]))}var Ct="apps/example/Dockerfile",Hn="app:trivy-scan",mr,dr,gr,fr,hr;var qn=f(()=>{k();de();mr=l({meta:{name:"build",description:`docker build -t ${Hn} (image for the trivy image scan)`},run(){if(!pr(Ct))console.warn(`\u26A0\uFE0F ${Ct} not found \u2014 skipping image build`),process.exit(0);if(!ur("docker"))console.warn("\u26A0\uFE0F docker not found \u2014 skipping image build"),process.exit(0);if(g(["docker","build","-t",Hn,"-f",Ct,"."])!==0)console.warn("\u26A0\uFE0F image build failed \u2014 skipping the Trivy image scan");process.exit(0)}}),dr=V({name:"fs",description:"trivy fs . --severity HIGH,CRITICAL (filesystem scan)",prefixArgs:["fs"],defaultArgs:[".","--severity","HIGH,CRITICAL"],spawn:St}),gr=V({name:"image",description:"trivy image <image> --severity HIGH,CRITICAL (container scan)",prefixArgs:["image"],spawn:St}),fr=l({meta:{name:"trivy",version:"1.0.0",description:"Trivy wrapper \u2014 vuln scanning, defensive (skips if binary missing)"},subCommands:{fs:dr,image:gr,build:mr},run(){let e=v("trivy");if(e.length===0)console.log(`
...
`),process.exit(0);process.exit(St(e))}}),hr=fr});var br;var Wn=f(()=>{k();br=l({meta:{name:"codeql",version:"1.0.0",description:"CodeQL wrapper \u2014 info and local guidance (CodeQL runs in GitHub Actions)"},run(){console.log(`
...
`;await G(n,s)}function _r(e){return{name:`${Xe(e)}/native-crates`,version:"0.0.0",private:!0,scripts:{build:"m native build --pure",test:"m native test --pure","cargo:check":"m native check --pure","cargo:clippy":"m native clippy --pure","cargo:fmt":"m native fmt --pure","cargo:fmt:check":"m native fmt:check --pure"}}}async function Nt(e,t={}){let n=O(e,"packages","native","crates");return await Ze(n,{recursive:!0}),await G(O(n,"package.json"),`${JSON.stringify(_r(t),null,2)}
...
`).filter(Boolean))try{let m=await Gt(b).text();if(m.includes(t))await _e(b,m.replaceAll(t,e))}catch{}}n++,console.log(`  \u2713 ${c.name}/`)}else if(c.isFile()&&c.name.endsWith(".md")){let p=c.name.replace(/\.md$/,""),w=ae(M,p);await Ee(w,{recursive:!0});let b=await Gt(u).text();if(e!==t)b=b.replaceAll(t,e);if(b.startsWith("---"))await _e(ae(w,"SKILL.md"),b);else{let C=`---
name: ${p}
description: ${p} skill
---
...
`)}function Xo(){return new Set(ee.map((e)=>e.dir))}function Ft(e){return Ge("ci",...e.split("/"))}function sc(e,t){if(t==="ci.steps.yml")return e.startsWith("sections/");return(e.split("/").pop()??"")===t}async function ic(e,t){let n=[];for(let o of ee){if(!t.has(o.dir))continue;for(let s of o.ciFiles??[]){if(!sc(s,e))continue;n.push((await ve(Ft(s)).text()).trimEnd())}}return n}async function rc(e,t){for(let n of ee){if(!t.has(n.dir))continue;for(let o of n.ciFiles??[])if((o.split("/").pop()??"")===e)return Ft(o)}return null}async function ac(){let e=Ft("ci.bootstrap.yml");if(!await ve(e).exists())return"";return(await ve(e).text()).trimEnd()}async function cc(e,t){if(!t)return;let n=Mo(ee,t),o=await Go(e);return(s)=>{let{content:i}=Uo(s,n);return o?i.replaceAll(Po,o):i}}async function lc(e,t,n,o={}){let s=o.enabled??Xo(),i=await rc(t,s);if(!i){console.log(`\u26A0\uFE0F Skipping ${t} \u2014 no enabled feature declares it`);return}let r=await ve(i).text(),a=await ic(n,s),c=a.join(`
