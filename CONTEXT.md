...
- Manifests are edited as text through the shared editor in `packages/tooling/src/manifest`, so adding or removing turbo tasks and workspace entries leaves the surrounding formatting alone; a manifest that does not parse is left untouched instead of being rewritten.
...
| Biome | 0 errors; 14 warnings + 10 infos over 158 files (incl. the coverage fragments, previously pruned) |
...
- `packages/tooling/skills` ships `SKILL.md` sources that are installed into `.agents/skills/`
<!-- TEMPLATE-ONLY:START(template) -->
